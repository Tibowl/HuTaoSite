...
const Wrapped = forwardRef(function FLink({ children, onClick, href, font = "semibold", size = "xl", className = "", location }: any, innerRef: ForwardedRef<HTMLAnchorElement>) {
    let colors = "text-slate-900 dark:text-blue-100 hover:text-blue-800 dark:hover:text-blue-400"
    if (href == location)
        colors = "text-blue-800 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-500"
    else if (location.startsWith(href) && href != "/")
        colors = "text-blue-700 dark:text-blue-300 hover:text-blue-900 dark:hover:text-blue-400"
